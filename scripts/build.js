...
function getPackages(packagePattern, tsOnly = false) {
  ...
  if (pattern === '*' && !tsOnly) {
    return `@sspingme/!(${[...META_PACKAGES].join('|')})`;
  }
  if (!pattern.includes('*')) {
    pattern = `*${pattern}`;
  }
  const packages = [
    ...new Set(
      fastGlob
        .sync([
          `./node_modules/@sspingme/${pattern}/src/**/*.${
...
if (shouldLint) {
  run(`npm run lint --fix plugins/${scope}/{src,test}`);
}

if (shouldCleanup) {
  // these modules will be installed by `npm link` but not useful for actual build
  const dirtyModules =
    'node_modules/@types/react,node_modules/sspingme';
